    ...
    for encoding in encodings:
        try:
            with open(arquivo, 'r', encoding=encoding) as f:
                ...
                # Verifica se usa TAB, vírgula ou ponto e vírgula
                ...
                # Remove espaços em branco dos cabeçalhos se houver
                ...
                for row in reader:
                    ...
                    tag_name = row.get('Tag_name', row.get('tag_name', 
                               row.get('TagName', row.get('tagname', 
                               row.get('Tag', row.get('tag', '')))))).strip()
                    
                    # Ignora linhas vazias
                    if not channel or not device or not tag_name:
                        print(f"⚠️  Linha ignorada (falta channel, device ou tag_name): {dict(row)}")
                        continue
...
if __name__ == "__main__":
    ...
    criar_tags_do_arquivo(
        ...
        data_type=-1,  # Default - herda do CLP automaticamente -1 = default CLP
